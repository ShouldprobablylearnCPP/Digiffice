...
{
    public class PeercomputeManager
    {
        // Class Variables (P2P)

        // Class Variables (General)
        int peercomputeType = 0; // 0 = None (Will cause error) | 1 = P2P | 2 = Client-Server (Not yet implemented)
        WPFDataGrid globalDataGrid;
        string txtFileDescription = "Plain Text Document";
        string dgpdFileDescription = "Digiffice Peercompute Data File";
        string dgpuFileDescription = "Digiffice Peercompute Userlist File";
        string mp3FileDescription = "MP3 Audio File";
        string unkownFileDescription = "Unknown File Type";
        string directoryDescription = "Folder";

        public string GetPeercomputeType(string PeercomputeDirPath)
        {
            // Find data file in directory
            string dataFilePath = @$"{PeercomputeDirPath}/PEERCOMPUTE_DATA.dgpd";
            using (FileStream fs = File.OpenRead(dataFilePath))
            {
                using (StreamReader reader = new StreamReader(fs))
                {
                    // Find |PEERCOMPUTE_TYPE: ... in data file and get the value after the colon
                    string dataLine = File.ReadLines(dataFilePath).FirstOrDefault(line => line.StartsWith("|PEERCOMPUTE_TYPE:"));
                    if (dataLine == null)
                    {
                        throw new InvalidDataException("PEERCOMPUTE_TYPE not found in data file in Peercompute directory: " + PeercomputeDirPath);
                    }
                    else
                    {
                        return dataLine["|PEERCOMPUTE_TYPE:".Length..].Trim();
                    }
                }
            }
        }
        ...
        public void MapPeercompute(string PeercomputeDirectory, Control parentControl)
        {
            ...
            peercomputeType = PeercomputeType switch
            {
                "P2P" => 1,
                "Client-Server" => 2,
                _ => throw new InvalidDataException("Invalid Peercompute type in data file in Peercompute directory: " + PeercomputeDirectory)
            };
        ...
        {
            // Get List of Files and Directories in the Peercompute directory
            string[] files = Directory.GetFiles(PeercomputeDirectory);
            string[] directories = Directory.GetDirectories(PeercomputeDirectory);
            ...
            foreach (var item in directories)
            {
                ...
                newRow.MouseDoubleClick += PeercomputeMap_DirectoryDoubleClicked;
        ...
        public void PeercomputeMap_DirectoryDoubleClicked(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    MapP2PPeercomputeSubdirectory(globalDataGrid, ((dynamic)senderRow.Item).ParentDir + "\\" + ((dynamic)senderRow.Item).Name, ((dynamic)senderRow.Item).Nest + 1);
                }
                else if (peercomputeType == 2)
                {
                    // Not yet implemented
                }
                else
